...
# ============================================================================
# Function: TagAssignmentChargeback1
...
# - NewCostCenter  : CostCenter value to apply as Azure tag.
...
function New-AuditRow {
    ...
    return [pscustomobject][ordered]@{
        ...
        FunctionName = "TagAssignmentChargeback1"
        Action = $Action
        VmName = $VmName
        ...
        if ($requestRows.Count -eq 0) {
            ...
            foreach ($row in $requestRows) {
                ...
                try {
                    ...
                    $requestedCostCenter = Get-RowValue -Row $row -Column "NewCostCenter"
                    ...
                    if ([string]::IsNullOrWhiteSpace($requestedCostCenter) -and [string]::IsNullOrWhiteSpace($requestedUser)) {
                        Set-RequestResult -Row $row -Status "Skipped" -Message "No request values provided. Enter NewAssignedUser and/or NewCostCenter, then set RequestStatus=Submit."
