...
# ============================================================================
# Function: AvdInventoryReconcile
...
# NewCostCenter, RequestStatus, RequestMessage
#
...
function New-AuditRow {
    ...
    return [pscustomobject][ordered]@{
        ...
        FunctionName = "AvdInventoryReconcile"
        Action = $Action
        VmName = $VmName
    ...
    try {
        ...
        foreach ($discovered in $discoveredRows) {
            ...
            if ($existingByVmId.ContainsKey($key)) {
                ...
                if ($currentChanged) {
                    $csvChanged = $true
                    $auditRows.Add((New-AuditRow -Action "InventoryUpdated" -VmName (Get-RowValue -Row $merged -Column "VmName") -VmResourceId $vmId -HostPoolName (Get-RowValue -Row $merged -Column "HostPoolName") -Result "Success" -Message "One or more current-state fields changed."))
                }
            }
            else {
                $csvChanged = $true
                $outputRows.Add($discovered)
                $auditRows.Add((New-AuditRow -Action "InventoryAdded" -VmName (Get-RowValue -Row $discovered -Column "VmName") -VmResourceId $vmId -HostPoolName (Get-RowValue -Row $discovered -Column "HostPoolName") -Result "Success" -Message "New AVD session host VM added to CSV."))
            }
        }
        ...
        foreach ($existing in $existingRows) {
            ...
            if (-not $seenVmIds.Contains($existingVmId)) {
                $csvChanged = $true
                $auditRows.Add((New-AuditRow -Action "InventoryRemoved" -VmName (Get-RowValue -Row $existing -Column "VmName") -VmResourceId $existingVmId -HostPoolName (Get-RowValue -Row $existing -Column "HostPoolName") -Result "Success" -Message "Row removed because the VM is no longer a live AVD session host discovered by this automation."))
            }
        }

        if ($csvChanged) {
            Set-BlobText -BlobName $inventoryBlob -Text (Convert-RowsToCsvText -Rows $outputRows) -LeaseId $leaseId
            Write-Stage -Stage "WRITE" -Message "Inventory CSV updated because real inventory changes were detected."
        }
        else {
