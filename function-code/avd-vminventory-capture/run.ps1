...
# VmName, Location, VmResourceId, ResourceGroup, HostPoolName, HostPoolResourceGroup,
...
function Test-ResourceInAutomationRegion {
    param(
    ...
    return $ResourceLocation.Trim().Equals($AutomationRegion, [System.StringComparison]::OrdinalIgnoreCase)
}
...
function New-DiscoveredInventoryRow {
    ...
    Set-RowValue -Row $row -Column "Location" -Value $Vm.Location
...
try {
    ...
    Write-Stage -Stage "CONFIG" -Message "AutomationRegion=$automationRegion; InputCsvBlob=$inventoryBlob; StorageAccount=$(Get-RequiredSetting -Name 'STORAGE_ACCOUNT_NAME'); Container=$(Get-RequiredSetting -Name 'STORAGE_CONTAINER_NAME')."
    Ensure-CsvBlobExists -BlobName $inventoryBlob -Columns $CsvColumns | Out-Null

    Write-Stage -Stage "DISCOVERY" -Message "Loading Azure VMs for region '$automationRegion'."
    $allVms = @(
        Get-AzVM -ErrorAction Stop | Where-Object {
            Test-ResourceInAutomationRegion -ResourceLocation $_.Location -AutomationRegion $automationRegion
        }
    )
    Write-Stage -Stage "DISCOVERY" -Message "Loaded $($allVms.Count) VM(s) from region '$automationRegion'."
    ...
    $hostPools = @(
        $hostPools | Where-Object {
            Test-ResourceInAutomationRegion -ResourceLocation $_.Location -AutomationRegion $automationRegion
        }
    )
    Write-Stage -Stage "DISCOVERY" -Message "Loaded $($hostPools.Count) host pool(s) in region '$automationRegion' before applying host-pool filters."
    ...
    foreach ($hostPool in $hostPools) {
        ...
        foreach ($sessionHost in $sessionHosts) {
            ...
            try {
                ...
                if (-not (Test-ResourceInAutomationRegion -ResourceLocation $vm.Location -AutomationRegion $automationRegion)) {
                    Write-Stage -Stage "REGION" -Message "Skipping VM '$($vm.Name)' because location '$($vm.Location)' does not match '$automationRegion'."
                    continue
                }
