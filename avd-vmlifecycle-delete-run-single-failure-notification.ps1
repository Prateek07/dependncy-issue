...
function Initialize-Settings {
    ...
    Write-LogInfo "VM scope is controlled by AZURE_SUBSCRIPTION_ID and AUTOMATION_REGION."
...
function Send-CleanupTeamsNotification {
    ...
    $normalizedStatus = if ([string]::IsNullOrWhiteSpace($Status)) {
        'Failed'
    }
    else {
    ...
    $title = "AVD VM lifecycle cleanup - $normalizedStatus"

    $statusColor = if ($normalizedStatus -eq 'Succeeded') {
        'Good'
    }
    else {
        'Attention'
    ...
    Write-Host "[TEAMS] [$timestampUtc] Starting Teams notification processing. VmName='$VmName'; Status='$normalizedStatus'; Color='$statusColor'."
    ...
    foreach ($row in $statusRows) {
        ...
        if ($row.Label -in @('Entra Deletion', 'Intune Deletion')) {
            if ($valueText -match '^(Succeeded|Deleted)') {
                $valueColor = 'Good'
            }
            elseif ($valueText -match '^(Failed|Error)') {
                ...
                @{
                    ...
                    items = @(
                        @{
                            ...
                            text   = $valueText
                            color  = $valueColor
                            weight = if ($valueColor -ne 'Default') { 'Bolder' } else { 'Default' }
                            wrap   = $true
                        }
                    )
                }
...
function Send-CleanupIssueNotification {
    param(
        ...
        [string] $SubscriptionId = 'Unknown',
        [string] $Region = 'Unknown',
        [string] $EntraDeletionStatus = 'Not processed',
        [string] $IntuneDeletionStatus = 'Not processed'
    )

    Write-Host "[TEAMS] Preparing cleanup error notification. VmName='$VmName'; ResourceGroup='$ResourceGroup'; Details='$Details'."

    $notificationSent = Send-CleanupTeamsNotification `
        -Status 'Failed' `
        -VmName $VmName `
    ...
    if (-not $notificationSent) {
        Write-Host "[WARNING] Teams cleanup error notification was not delivered. The original cleanup result is unchanged."
    }
}
...
function Process-DeletedVmEvent {
    ...
    if (-not (Test-IsDeletedResourceEvent -EventType $eventType)) {
        $issueMessage = "Skipped event because it is not a supported VM deleted event. EventType='$eventType'."
        Write-LogWarning $issueMessage
        Send-CleanupIssueNotification -Details $issueMessage -Region $script:AutomationRegion
        return
    }

    if ([string]::IsNullOrWhiteSpace($subject)) {
        $issueMessage = "Skipped event because subject/resource ID is empty. EventId='$eventId'."
        Write-LogWarning $issueMessage
        Send-CleanupIssueNotification -Details $issueMessage -Region $script:AutomationRegion
        return
    }
    ...
    if ($null -eq $vmInfo) {
        $issueMessage = "Skipped event because subject is not a Microsoft.Compute/virtualMachines resource ID. Subject='$subject'."
        Write-LogWarning $issueMessage
        Send-CleanupIssueNotification -Details $issueMessage -Region $script:AutomationRegion
        return
    }
    ...
    if (-not [string]::IsNullOrWhiteSpace($eventResourceLocation)) {
        ...
        if (-not $eventResourceLocation.Equals(
            ...
            Write-LogInfo (
                "[REGION] Ignoring VM deletion event because event location " +
                "'$eventResourceLocation' does not match Function App region " +
                "'$script:AutomationRegion'. No Entra/Intune cleanup or Teams notification will be performed. " +
                "VmName='$($vmInfo.VmName)'; ResourceGroup='$($vmInfo.ResourceGroup)'."
            )
            return
        }
        ...
        Write-LogInfo "[REGION] Event payload does not contain resource location. Using resource-group regional validation."
        ...
        if (-not $resourceGroupMatchesRegion) {
            Write-LogInfo (
                "[REGION] Ignoring VM deletion event because resource group " +
                "'$($vmInfo.ResourceGroup)' does not belong to Function App region " +
                "'$script:AutomationRegion'. No Entra/Intune cleanup or Teams notification will be performed. " +
                "VmName='$($vmInfo.VmName)'."
            )
            return
    ...
    if (-not $vmInfo.SubscriptionId.Equals($script:TargetSubscriptionId, [System.StringComparison]::OrdinalIgnoreCase)) {
        $issueMessage = "Skipped VM because subscription '$($vmInfo.SubscriptionId)' does not match configured subscription '$script:TargetSubscriptionId'."
        Write-LogWarning "$issueMessage VmName='$($vmInfo.VmName)'."
        Send-CleanupIssueNotification `
    ...
    if (-not $script:DeleteEntraDevice -and -not $script:DeleteIntuneDevice) {
        $issueMessage = "Both DELETE_ENTRA_DEVICE and DELETE_INTUNE_DEVICE are false. No device cleanup was performed."
        Write-LogWarning "$issueMessage VmName='$($vmInfo.VmName)'."
        Send-CleanupIssueNotification `
            -Details $issueMessage `
            -VmName $vmInfo.VmName `
            ...
            -SubscriptionId $vmInfo.SubscriptionId `
            -Region $notificationRegion `
            -EntraDeletionStatus 'Disabled' `
            -IntuneDeletionStatus 'Disabled'
        return
    }
    ...
    if ($entraMatchCount -gt $script:MaxMatchesToDelete -or $intuneMatchCount -gt $script:MaxMatchesToDelete) {
        $issueMessage = "Safety stop: matching device count exceeds MAX_MATCHES_TO_DELETE. EntraMatches='$entraMatchCount'; IntuneMatches='$intuneMatchCount'; MaximumAllowed='$script:MaxMatchesToDelete'."
        Write-LogWarning "$issueMessage VmName='$($vmInfo.VmName)'. Deletion skipped and Service Bus message will be completed."
        Send-CleanupIssueNotification `
            -Details $issueMessage `
            -VmName $vmInfo.VmName `
            -ResourceGroup $vmInfo.ResourceGroup `
            -SubscriptionId $vmInfo.SubscriptionId `
            -Region $notificationRegion `
            -EntraDeletionStatus 'Blocked by safety limit' `
            -IntuneDeletionStatus 'Blocked by safety limit'
        return
    }
...
$script:CurrentServiceBusDeliveryCount = 1

# Capture Service Bus metadata before settings initialization so configuration
# failures still have the correct message ID and delivery count.
$messageId = Get-PropertyValue -Object $TriggerMetadata -Names @('MessageId', 'messageId')
$deliveryCountValue = Get-PropertyValue -Object $TriggerMetadata -Names @('DeliveryCount', 'deliveryCount')

if (-not [string]::IsNullOrWhiteSpace([string] $messageId)) {
    $script:CurrentServiceBusMessageId = [string] $messageId
}

$parsedDeliveryCount = 1
if (-not [string]::IsNullOrWhiteSpace([string] $deliveryCountValue)) {
    $deliveryCountParsedSuccessfully = [int]::TryParse(
        [string] $deliveryCountValue,
        [ref] $parsedDeliveryCount
    )

    if (-not $deliveryCountParsedSuccessfully -or $parsedDeliveryCount -lt 1) {
        $parsedDeliveryCount = 1
    }
}

$script:CurrentServiceBusDeliveryCount = $parsedDeliveryCount

try {
    Initialize-Settings

    $messageId = $script:CurrentServiceBusMessageId
    $deliveryCount = $script:CurrentServiceBusDeliveryCount
    ...
    try {
        $deliveryCountForFailure = [int] $script:CurrentServiceBusDeliveryCount

        if ($deliveryCountForFailure -eq 1) {
            ...
            Write-Host "[TEAMS] Preparing the first and only failure notification. ServiceBusMessageId='$script:CurrentServiceBusMessageId'; DeliveryCount='$deliveryCountForFailure'; VmName='$failureVmName'; ResourceGroup='$failureResourceGroup'."
            ...
            Write-LogInfo (
                "[TEAMS] Skipping duplicate failure notification because this is a Service Bus retry. " +
                "ServiceBusMessageId='$script:CurrentServiceBusMessageId'; DeliveryCount='$deliveryCountForFailure'. " +
                "The failure notification was attempted only on DeliveryCount='1'."
            )
        }
    }
    catch {
