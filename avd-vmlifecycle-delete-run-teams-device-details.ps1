...
function Send-TeamsNotification {
    ...
    Write-Host "[TEAMS] [$timestampUtc] Teams notification is enabled and the webhook URL is configured."
    ...
    try {
        ...
        Write-Host "[TEAMS] [$timestampUtc] Sending notification to the configured Teams Workflows webhook."

        Invoke-RestMethod `
            -Method Post `
            -Uri $script:TeamsWebhookUrl `
        ...
        Write-Host "[TEAMS] [$successTimestampUtc] Teams notification sent successfully. Title='$Title'."
        Write-Information "Teams notification sent successfully. Title='$Title'." -InformationAction Continue
        return $true
    }
    catch {
        # Best-effort only: notification failure must never fail VM cleanup,
        # abandon the Service Bus message, or trigger a retry.
...
function Send-CleanupTeamsNotification {
    param(
        ...
        [string] $EntraOutcome = 'Not available',
        [string] $IntuneOutcome = 'Not available',
        [string] $Details = ''
    )

    $title = "AVD VM lifecycle cleanup - $Status"
    $lines = @(
        "Status: $Status",
        "VM name: $VmName",
        "Resource group: $ResourceGroup",
        "Subscription: $SubscriptionId",
        "Automation region: $script:AutomationRegion",
        "Entra result: $EntraOutcome",
        "Intune result: $IntuneOutcome",
        "Service Bus message ID: $script:CurrentServiceBusMessageId",
        "Delivery count: $script:CurrentServiceBusDeliveryCount"
    )
    ...
    return (Send-TeamsNotification -Title $title -Message ($lines -join "`n"))
}
...
try {
    ...
    try {
        ...
        $failureNotificationSent = Send-CleanupTeamsNotification `
            ...
            -EntraOutcome 'Cleanup did not complete' `
            -IntuneOutcome 'Cleanup did not complete' `
