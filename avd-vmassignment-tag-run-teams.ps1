...
function Send-TagAssignmentTeamsNotification {
    param(
        [Parameter(Mandatory)][string]$Status,
        ...
        [string]$RequestedCostCenter = "",
        [string]$ResultMessage = "",
        [Parameter(Mandatory)][bool]$Enabled,
        [AllowEmptyString()][string]$WebhookUrl
    )

    $assignedUserDisplay = if ([string]::IsNullOrWhiteSpace($AssignedUser)) { "Not assigned" } else { $AssignedUser }
    $costCenterDisplay = if ([string]::IsNullOrWhiteSpace($CostCenter)) { "Not set" } else { $CostCenter }
    $requestedUserDisplay = if ([string]::IsNullOrWhiteSpace($RequestedUser)) { "No user change requested" } else { $RequestedUser }
    $requestedCostCenterDisplay = if ([string]::IsNullOrWhiteSpace($RequestedCostCenter)) { "No cost center change requested" } else { $RequestedCostCenter }
    $resultDisplay = if ([string]::IsNullOrWhiteSpace($ResultMessage)) { "No result message available" } else { $ResultMessage }

    $title = "AVD VM tag and user assignment - $Status"
    $message = @(
        "Status: $Status",
        "VM name: $VmName",
        "Resource group: $ResourceGroup",
        "Host pool: $HostPoolName",
        "Automation region: $AutomationRegion",
        "Assigned user: $assignedUserDisplay",
        "Cost center: $costCenterDisplay",
        "Requested user value: $requestedUserDisplay",
        "Requested cost center value: $requestedCostCenterDisplay",
        "Result: $resultDisplay"
    ) -join "`n"

    Send-TeamsNotification -Title $title -Message $message -Enabled $Enabled -WebhookUrl $WebhookUrl | Out-Null
}
        ...
        if ($requestRows.Count -eq 0) {
            ...
            foreach ($row in $requestRows) {
                ...
                try {
                    ...
                    Write-Host "[TEAMS] Preparing tag-assignment notification for VM '$vmName'. Status='$finalStatus'; AssignedUser='$finalAssignedUser'; CostCenter='$finalCostCenter'."
                    Send-TagAssignmentTeamsNotification `
                        -Status $finalStatus `
                        -VmName $vmName `
                    ...
                    Write-Host "[TEAMS] Preparing failure notification for VM '$vmName'."
                    Send-TagAssignmentTeamsNotification `
                        -Status "Failed" `
                        -VmName $vmName `
                        -ResourceGroup $resourceGroupName `
