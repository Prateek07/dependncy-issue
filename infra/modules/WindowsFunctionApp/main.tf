resource "azurerm_storage_share" "function_content_share" {
  for_each = var.windowsFunctionAppVariables
 
  name                 = try(each.value.content_share_name, null)
  storage_account_id   = try(each.value.storage_account_id, null)
  quota                = try(each.value.content_share_quota, 5120)
}
resource "azurerm_windows_function_app" "windowsFunctionApp" {
  ...
  depends_on = [azurerm_storage_share.function_content_share]
