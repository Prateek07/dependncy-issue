...
resource "azurerm_virtual_machine_extension" "extend_os_disk" {
  ...
  settings = jsonencode({
    commandToExecute = "powershell.exe -ExecutionPolicy Bypass -Command \"Update-HostStorageCache; $p=Get-Partition -DriveLetter C; $s=Get-PartitionSupportedSize -DiskNumber $p.DiskNumber -PartitionNumber $p.PartitionNumber; if($s.SizeMax -gt $p.Size){Resize-Partition -DiskNumber $p.DiskNumber -PartitionNumber $p.PartitionNumber -Size $s.SizeMax}else{Write-Host 'C: drive already at maximum supported size.'}\""
  })
