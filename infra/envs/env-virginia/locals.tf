...
################################# subnetvariable ################################
    snetVariableslocal={
        for key, value in var.snetVariables : key => merge(value, {
...
################################# Private Endpoint Variables ################################
  privateEndpointTargetIds = {
    ...
    functionapp = { for k, v in module.WindowsFunctionApp.windowsFunctionApp : k => v.id }
    storageaccount = { for k, v in module.StorageAccount.storageAccount : k => v.id }
    servicebusnamespace = { for k, v in module.ServiceBusNamespace.serviceBusNamespace : k => v.id }
  }
 
  privateEndpointVariables = {
    for key, value in var.privateEndpointVariables : key => merge(value, {
...
################################# Diagnostic Settings Variables ################################
  diagnosticSettings = merge([
    for type, config in {
      ...
      keyvault  = { vars = var.keyVaultVariables,  module = module.Keyvault.Keyvault }
      servicebusnamespace = { vars = var.serviceBusNamespaceVariables, module = module.ServiceBusNamespace.serviceBusNamespace }
      functionapp = { vars = var.windowsFunctionAppVariables, module = module.WindowsFunctionApp.windowsFunctionApp }
      storageaccount = { vars = var.storageAccountVariables, module = module.StorageAccount.storageAccount }
      eventgridsystopic = { vars = var.eventGridSystemTopicVariables, module = module.EventGridSystemTopic.eventGridSystemTopic }
      appserviceplan = { vars = var.appServicePlanVariables, module = module.AppServicePlan.appServicePlan }
    } : {
...
################################# Generated Session Hosts Variables ################################
  generated_session_hosts = merge([
    for sh_key, sh_value in var.sessionHostVariables : {
      ...
      "${sh_key}-${sh_value.start_index + i}" => {
        ...
        location                     = sh_value.location
        resource_group_name          = sh_value.resource_group_name
        ...
        vtpm_enabled = try(sh_value.vtpm_enabled, null)
        secure_boot_enabled = try(sh_value.secure_boot_enabled, null)
      }
...
################################# Session Hosts Resolved Variables ################################
  session_hosts_resolved = {
    for key, value in local.generated_session_hosts : key => {
      ...
      vtpm_enabled = try(value.vtpm_enabled, null)
      secure_boot_enabled = try(value.secure_boot_enabled, null)
    }
  }
################################# Unregister Session Hosts Variables ################################
...
################################# Hostpool Registration Variables ################################
...
################################# DCR Variables Resolved ################################
  dcrVariablesResolved = {
    for key, value in var.dcrVariables : key => merge(
      value,
      {
        log_analytics_workspace_id = module.LogAnalyticsWorkspace.LogAnalyticsWorkspace[
          value.log_analytics_workspace_key
        ].id
  
        data_collection_endpoint_id = try(
          module.DCE.dce[value.data_collection_endpoint_key].id,
          null
        )
      }
    )
  ...
  dcrAssociationVariablesResolved = merge([
    for dcr_key, dcr_value in var.dcrVariables : {
      ...
      "${dcr_key}-${session_host_resource_key}" => {
        ...
        description = try(
          dcr_value.association_description,
          "DCR association to session host VM"
        )
      }
    }
  ]...)
################################# DCE Variables Resolved ################################
  dceVariablesResolved = {
    for key, value in var.dceVariables : key => value
  }
################################# DCE Association Variables  ################################
  dceAssociationVariablesResolved = merge([
    for dce_key, dce_value in var.dceVariables : {
      ...
      "${dce_key}-${session_host_resource_key}" => {
        ...
        description = try(
          dce_value.association_description,
          "DCE association to session host VM"
        )
      }
    }
  ]...)
