...
module "ComputeGalleryImageVersionData" {
  ...
  imageVersionDataVariables = var.imageVersionDataVariables
...
module "StorageAccount" {
  ...
  storageContainerVariables = var.storageContainerVariables
  depends_on = [
    module.ResourceGroup
  ]
}
...
#       source_resource_id = (
#         lower(value.topic_type) == "microsoft.resourcenotifications.resources"
#         ? "/subscriptions/${data.azurerm_client_config.current.subscription_id}"
#         : module.ResourceGroup.rg[value.source_resource_group_key].id
#       )
...
module "EventGridSystemTopic" {
  ...
  eventGridSystemTopicVariables = {
    for key, value in var.eventGridSystemTopicVariables : key => merge(value, {
      source_resource_id = (
        try(value.source_resource_id, null) != null
        ? value.source_resource_id
        : (
            lower(value.topic_type) == "microsoft.resourcenotifications.resources"
            ? "/subscriptions/${data.azurerm_client_config.current.subscription_id}"
            : (
                lower(value.topic_type) == "microsoft.storage.storageaccounts"
                ? module.StorageAccount.storageAccount[value.source_storage_account_key].id
                : module.ResourceGroup.rg[value.source_resource_group_key].id
              )) )
      identity_type = "UserAssigned"
      identity_ids = compact([
        ...
        try(value.virginia_identity_name, null) != null
        ? "/subscriptions/${data.azurerm_client_config.current.subscription_id}/resourceGroups/${value.virginia_identity_resource_group_name}/providers/Microsoft.ManagedIdentity/userAssignedIdentities/${value.virginia_identity_name}"
        : null
      ])
      # identity_ids = (
      #   try(value.identity_key, null) != null
      #   ? [module.ManagedIdentity.mIdentity[value.identity_key].id]
      #   : try(value.identity_ids, [])
      # )
    })
  }
} 
module "EventGridSubscription" {
  ...
  eventGridSubscriptionVariables = {
    ...
    //  system_topic                  = //module.EventGridSystemTopic.eventGridSystemTopic[value.system_topic_key].name
      ...
      advanced_filter = merge(
          value.advanced_filter != null
          ? value.advanced_filter
          : {},
          {
            string_begins_with = concat(
              try(value.advanced_filter.string_begins_with, []),length(value.resource_group_filter_names) > 0 ? [
                  {
                    key = "data.resourceInfo.id"
 
                    values = [
                      for resource_group_name in value.resource_group_filter_names :
                      "/subscriptions/${data.azurerm_client_config.current.subscription_id}/resourceGroups/${resource_group_name}/"
                    ]}]: []
            )})})
    }
}
...
module "WindowsFunctionApp" {
  ...
  windowsFunctionAppVariables = {
    for key, value in var.windowsFunctionAppVariables : key => merge(value, {
      ...
      storage_account_id                   = module.StorageAccount.storageAccount[value.storage_account_key].id
      ...
      app_settings = merge(
        value.app_settings,
        {
          ...
          WEBSITE_CONTENTAZUREFILECONNECTIONSTRING      = module.StorageAccount.storageAccount[value.storage_account_key].primary_connection_string
          AzureWebJobsStorage                           = module.StorageAccount.storageAccount[value.storage_account_key].primary_connection_string 
          APPLICATIONINSIGHTS_CONNECTION_STRING        = module.ApplicationInsights.application_insights_connection_strings[value.application_insights_resource_key]
          APPINSIGHTS_INSTRUMENTATIONKEY                      = module.ApplicationInsights.application_insights_instrumentation_keys[value.application_insights_resource_key]
          AZURE_SUBSCRIPTION_ID = data.azurerm_client_config.current.subscription_id
          AZURE_TENANT_ID       = data.azurerm_client_config.current.tenant_id
        }
      )
      virtual_network_subnet_id = (
        try(value.subnet_key, null) != null
        ? module.Subnet.snet[value.subnet_key].id
        : null
      )
    })
  }
