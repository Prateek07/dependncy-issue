...
module "EventGridSystemTopic" {
  ...
  eventGridSystemTopicVariables = {
    for key, value in var.eventGridSystemTopicVariables : key => merge(value, {
      ...
      identity_type = "UserAssigned"
      identity_ids = [ module.ManagedIdentity.mIdentity[value.identity_key].id, "/subscriptions/${data.azurerm_client_config.current.subscription_id}/resourceGroups/${value.virginia_identity_resource_group_name}/providers/Microsoft.ManagedIdentity/userAssignedIdentities/${value.virginia_identity_name}" ]
      # identity_ids = (
      #   try(value.identity_key, null) != null
      #   ? [module.ManagedIdentity.mIdentity[value.identity_key].id]
