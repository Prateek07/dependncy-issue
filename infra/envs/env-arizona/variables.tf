...
variable "eventGridSystemTopicVariables" {
  ...
  type = map(object({
    ...
    topic_type                = string //optional(string, "Microsoft.Resources.ResourceGroups")
    identity_key              = optional(string)
    identity_type             = optional(string)
    identity_ids              = optional(list(string), [])
    virginia_identity_resource_group_name = optional(string)
    virginia_identity_name = optional(string)
    source_storage_account_key = optional(string)
    timeouts = optional(object({
      create = optional(string)
      read   = optional(string)
      update = optional(string)
...
variable "eventGridSubscriptionVariables" {
  ...
  type = map(object({
    ...
    resource_group_filter_names = optional(list(string), [])
...
variable "windowsFunctionAppVariables" {
  ...
  type = map(object({
    ...
    content_share_quota = optional(number, 5120)
