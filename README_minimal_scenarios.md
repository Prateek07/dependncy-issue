...
This repository includes event-driven automation in addition to the core Azure Virtual Desktop resources. The automation captures approved Azure events, applies Event Grid filters, stores matching events in Service Bus, and processes them with PowerShell functions running in a Windows Function App.

### 15.1 Automation Architecture

```text
Timer schedule every five minutes
        |
        +------------------------------------------+
                                                   |
VM deletion or approved CSV blob upload            |
        |                                          |
        v                                          |
Event Grid System Topic                            |
        |                                          |
        v                                          |
Event Grid Subscription and advanced filters       |
        |                                          |
        v                                          |
Service Bus Queue                                  |
        |                                          |
        +--------------------+---------------------+
                             |
                             v
                  Windows PowerShell Function App
                             |
                             +--> Managed Identity for Azure and Microsoft Graph access
                             +--> Storage Account for Function runtime and CSV processing
                             +--> Application Insights for execution telemetry
```

The repository supports these automation paths:

| Automation | Trigger | Service Bus queue | Function action |
|---|---|---|---|
| AVD inventory capture | Timer trigger every five minutes. | Not applicable. | Discovers live AVD session hosts and reconciles the master inventory CSV only when inventory changes are detected. |
| VM lifecycle automation | Azure resource deletion notification filtered to `Microsoft.Compute/virtualMachines`. | `sbq-avd-vmlc-automation-...` | Processes VM deletion and performs the configured Entra device cleanup flow. |
| VM assignment and tagging automation | Storage blob-created event filtered to the approved master CSV path. | `sbq-avd-vmat-automation-...` | Processes assignment, unassignment, tagging, tag removal, and status tracking instructions from CSV. |

### 15.2 Managed Identity

The user-assigned managed identity is attached to the Function App and can also be used as the Event Grid delivery identity. It avoids storing Azure, Storage, Service Bus, or Microsoft Graph credentials in source code.

Example configuration in `infra/envs/<env>/terraform.tfvars`:
...
Required permissions must be assigned through the approved RBAC and Microsoft Graph permission process. Typical permissions include Service Bus data receive/send, Storage data access, Azure resource access, and the Microsoft Graph application permission required by the Entra device cleanup function.

### 15.3 Service Bus Namespace and Queues

The Service Bus namespace provides the messaging boundary for AVD automation. Separate queues isolate VM lifecycle processing from CSV assignment and tagging processing. Retry and dead-letter settings prevent messages from being silently lost when the Function App cannot process them successfully.

#### 15.3.1 Create a New Service Bus Queue

Use this scenario when a new automation function requires a separate queue.

**Steps**

1. Open `infra/envs/<env>/terraform.tfvars`.
2. Add a new entry under `serviceBusQueueVariables`.
3. Use the existing Service Bus Namespace key.
4. Reference the new queue in the Event Grid Subscription and Function trigger.
...
serviceBusQueueVariables = {
  az-avd-sbq-prod-3 = {
    name                                 = "sbq-avd-sh-compliance-usgovaz-p01"
...
- Confirm the queue is created in the expected namespace.
- Confirm Event Grid sends messages to it.
- Confirm the Function App processes the message.

#### 15.3.2 Update an Existing Service Bus Queue

Update the existing queue entry in `serviceBusQueueVariables` and review the Terraform plan before deployment.
...
### 15.4 App Service Plan

The App Service Plan supplies Windows compute capacity for the Function App. The current pattern uses Elastic Premium `EP1`, which supports VNet integration and private networking.
...
### 15.5 Application Insights

Application Insights captures Function invocation traces, exceptions, dependencies, and execution results. It is linked to the environment Log Analytics Workspace through its logical key.
...
### 15.6 Storage Account and Container

The Storage Account supports the Function runtime and the CSV-driven automation. The container remains private, and public network access is disabled in the current pattern.
...
### 15.7 Event Grid System Topics

One system topic captures Azure resource lifecycle notifications, and another captures Storage Account blob events.

```hcl
eventGridSystemTopicVariables = {
  az-avd-egst-prod-1 = {
    name                      = "egst-avd-vmlc-automation-usgov-p01"
    resource_group_name       = "rg-avd-automation-usgovaz-p01"
    location                  = "Global"
    source_resource_group_key = "az-avd-rg-prod-4"
    topic_type                = "Microsoft.ResourceNotifications.Resources"
    identity_key              = "az-avd-mi-prod-1"
    tags                      = {}
  }
  az-avd-egst-prod-2 = {
    name                       = "egst-avd-vmat-automation-usgovaz-p01"
    resource_group_name        = "rg-avd-automation-usgovaz-p01"
    location                   = "usgovarizona"
    source_storage_account_key = "az-avd-sa-prod-1"
...
### 15.8 Event Grid Subscriptions and Filters

Event Grid Subscriptions select approved events from a System Topic and deliver matching events to a Service Bus Queue. Filters are important because they prevent unrelated Azure events or blob uploads from triggering the automation functions.

#### 15.8.1 Create a New Event Grid Subscription

Use this scenario when a new Azure event must be routed to a Service Bus Queue.

**Steps**

1. Confirm the Event Grid System Topic and Service Bus Queue already exist.
2. Open `infra/envs/<env>/terraform.tfvars`.
3. Add a new entry under `eventGridSubscriptionVariables`.
4. Configure the event type, destination queue, identity, and filter.
...
eventGridSubscriptionVariables = {
  az-avd-egs-prod-3 = {
    name                  = "egs-avd-hp-delete-usgovaz-p01"
    resource_group_name   = "rg-avd-automation-usgovaz-p01"
    ...
    advanced_filter = {
      string_in = [{
        ...
        values = ["Microsoft.DesktopVirtualization/hostPools"]
      }]
    }
  }
}
...
- Trigger one matching event and confirm the Function App runs.
- Trigger one non-matching event and confirm no message is sent.

#### 15.8.2 Update an Existing Event Grid Subscription

Update the required event type, queue key, or filter in the existing entry and validate both matching and non-matching events.
...
### 15.9 Windows Function App

The Windows Function App uses PowerShell, the App Service Plan, Storage Account, managed identity, Service Bus namespace, Application Insights, and a delegated VNet integration subnet. Environment-level keys are resolved to Azure resource IDs in `main.tf`.
...
windowsFunctionAppVariables = {
  az-avd-fa-prod-1 = {
    ...
    content_share_name            = "func-avd-automation-usgovaz-p01"
    ...
    app_settings = {
      ...
      SERVICEBUS_QUEUE_NAME                 = "sbq-avd-vmlc-automation-usgovaz-p01"
      TAG_ASSIGNMENT_QUEUE_NAME             = "sbq-avd-vmat-automation-usgovaz-p01"
      INPUT_CSV_BLOB                        = "avd-vm-master.csv"
      STATUS_CSV_BLOB                       = "state/avd-vm-processing-status.csv"
      STORAGE_ACCOUNT_NAME                  = "stavdfausgovazp01"
      STORAGE_CONTAINER_NAME                = "avd-vmat-automation"
      AUTOMATION_REGION                     = "usgovarizona"
      DRY_RUN                               = "false"
      ALLOW_ALL_VM_NAMES_IN_SUBSCRIPTION    = "true"
    }
...
The deployed Function project contains three functions:

```text
avd-vminventory-capture   -> Timer trigger every five minutes -> Reconciles live AVD inventory with the master CSV
avd-vmassignment-tag      -> Service Bus trigger              -> Processes submitted assignment and tagging requests
avd-vmlifecycle-delete    -> Service Bus trigger              -> Processes VM deletion and Entra cleanup
```

| Path | Purpose |
|---|---|
| `function-code/host.json` | Defines Azure Functions host behavior, extension bundle, timeout, and logging. |
| `function-code/profile.ps1` | Runs PowerShell profile initialization for the Function App. |
| `function-code/requirements.psd1` | Declares required PowerShell managed dependencies. |
| `function-code/<function-name>/function.json` | Defines the trigger and binding for each function. |
| `function-code/<function-name>/run.ps1` | Contains the PowerShell processing logic. |
| `scripts/package-functionapp-code.py` | Packages the Function project for deployment. |
| `scripts/deploy-functionapp-code.sh` | Deploys the generated package to the selected Function App. |
| `workflows/deploy-function.yml` | Selects the target region and deploys Function code through the approved runner. |

### 15.11 Automation Deployment Order

1. Deploy the Resource Group, subnets, Log Analytics Workspace, and Storage prerequisites.
2. Deploy the Managed Identity, Service Bus Namespace, App Service Plan, and Application Insights.
3. Deploy the Service Bus Queues and Storage container.
4. Deploy Event Grid System Topics and Event Grid Subscriptions.
5. Deploy the Windows Function App and private networking dependencies.
6. Grant required Azure RBAC and Microsoft Graph application permissions.
7. Package and deploy the PowerShell Function code.
8. Run one controlled test for each automation path and review Application Insights and Service Bus results.

> [!IMPORTANT]
> Do not test VM deletion automation against a production session host until Event Grid filters, managed identity permissions, Function App settings, queue processing, and `DRY_RUN` behavior have been validated in a controlled environment.
