...
# - new_az_res_costcenter: requested tag value, or REMOVE/UNASSIGN/CLEAR/DELETE to remove the az_res_costcenter tag.
# - RequestStatus  : Submit or Submitted to ask the function to process the row.
#
# Function-managed result columns:
# - AssignedUser, az_res_costcenter, RequestStatus, RequestMessage.
#
...
function Get-AutomationRegion {
    $region = Get-RequiredSetting -Name "AUTOMATION_REGION"
...
function Assert-VmInAutomationRegion {
    param(
        [Parameter(Mandatory)]$Vm,
        [Parameter(Mandatory)][string]$AutomationRegion
    )

    $vmLocation = Get-StringValue -Value $Vm.Location
    if ([string]::IsNullOrWhiteSpace($vmLocation)) {
        throw "Unable to determine the Azure location for VM '$($Vm.Name)'."
    }
    if (-not $vmLocation.Equals($AutomationRegion, [System.StringComparison]::OrdinalIgnoreCase)) {
        throw "VM '$($Vm.Name)' belongs to location '$vmLocation', but this Function App processes only '$AutomationRegion'."
    }
}
...
function Update-CostCenterTagTargets {
    ...
    if (Test-RemoveToken -Value $RequestedCostCenter) {
        Write-Host "[TAG] Remove token '$RequestedCostCenter' detected. Removing az_res_costcenter tag from $($targetIds.Count) resource(s)."
        ...
        foreach ($resourceId in $targetIds) {
            ...
            if ($null -ne $resource.Tags) {
                foreach ($tagKey in $resource.Tags.Keys) {
                    if ($tagKey -ieq "az_res_costcenter") {
            ...
            if (-not $costCenterFound) {
                Write-Host "[TAG] az_res_costcenter tag is already absent on '$resourceId'."
                continue
            }

            # Delete only the existing az_res_costcenter key/value pair.
            # Using Replace with an empty hashtable fails when this is the only tag.
            $existingCostCenterValue = ""
            foreach ($tagKey in $resource.Tags.Keys) {
                if ($tagKey -ieq "az_res_costcenter") {
                    $existingCostCenterValue = [string]$resource.Tags[$tagKey]
                    break
                }
            }

            if ([string]::IsNullOrWhiteSpace($existingCostCenterValue)) {
                Write-Warning "[TAG] The az_res_costcenter tag exists on '$resourceId', but its value is empty. Removing all tags is not attempted automatically."
                throw "Cannot safely remove az_res_costcenter from '$resourceId' because its existing value is empty."
            }

            $deleteTag = @{
                "az_res_costcenter" = $existingCostCenterValue
            }

            Write-Host "[TAG] Deleting az_res_costcenter='$existingCostCenterValue' from '$resourceId'."
            ...
            if (-not [string]::IsNullOrWhiteSpace($actualAfterRemoval)) {
                throw "az_res_costcenter tag removal verification failed for '$resourceId'. Azure returned '$actualAfterRemoval'."
            }

            Write-Host "[TAG] az_res_costcenter tag removal verified for '$resourceId'."
        }

        return "az_res_costcenter tag removed from $($targetIds.Count) resource(s)."
    }

    $normalizedCostCenter = $RequestedCostCenter.Trim()
    Write-Host "[TAG] Applying az_res_costcenter='$normalizedCostCenter' to $($targetIds.Count) resource(s)."
    foreach ($resourceId in $targetIds) {
        Update-AzTag -ResourceId $resourceId -Tag @{ "az_res_costcenter" = $normalizedCostCenter } -Operation Merge -ErrorAction Stop | Out-Null
    }
    return "az_res_costcenter tag '$normalizedCostCenter' applied to $($targetIds.Count) resource(s)."
}
...
function Get-ResourceCostCenterTag {
    ...
    foreach ($key in $resource.Tags.Keys) {
        if ($key -ieq "az_res_costcenter") { return Get-StringValue -Value $resource.Tags[$key] }
    }
    return ""
}
...
function Get-BoolSetting {
    ...
    $rawValue = Get-OptionalSetting -Name $Name
    ...
    switch ($rawValue.Trim().ToLowerInvariant()) {
        ...
        "off"   { return $false }
        default {
            Write-Warning "App setting '$Name' has unsupported Boolean value '$rawValue'. Using default '$DefaultValue'."
            return $DefaultValue
        }
    }
}
...
function Send-TeamsNotification {
    param(
        ...
        [AllowEmptyString()][string]$WebhookUrl
    )

    $timestampUtc = (Get-Date).ToUniversalTime().ToString("yyyy-MM-dd HH:mm:ssZ")
    Write-Host "[TEAMS] [$timestampUtc] Starting Teams notification processing. Title='$Title'."

    if (-not $Enabled) {
        Write-Host "[TEAMS] [$timestampUtc] Teams notification is disabled. Tag-assignment processing is not affected."
        return $false
    }

    if ([string]::IsNullOrWhiteSpace($WebhookUrl)) {
        Write-Host "[WARNING] [$timestampUtc] Teams notification was not sent because TEAMS_WEBHOOK_URL is empty."
        Write-Warning "Teams notification was not sent because TEAMS_WEBHOOK_URL is not configured. Tag-assignment processing will continue."
        return $false
    }
            ...
            @{
                ...
                content     = @{
                    ...
                    body      = @(
                        ...
                        @{ type = "TextBlock"; text = "Timestamp UTC: $timestampUtc"; isSubtle = $true; wrap = $true; spacing = "Small" }
                    )
                }
            }
        )
    ...
    try {
        Write-Host "[TEAMS] [$timestampUtc] Building Adaptive Card payload."
        $payloadJson = $payload | ConvertTo-Json -Depth 30 -Compress
        Write-Host "[TEAMS] [$timestampUtc] Sending Teams notification."
        ...
        Write-Host "[TEAMS] Teams notification sent successfully. Title='$Title'."
        return $true
    }
    catch {
        $notificationError = $_.Exception.Message
        Write-Host "[WARNING] Teams notification could not be delivered. Error='$notificationError'."
        Write-Host "[WARNING] Tag and user assignment processing is not failed because Teams notification is best-effort only."
        Write-Warning "Teams notification could not be delivered. The Service Bus message and assignment result will not fail because of this notification error. Error='$notificationError'."
        return $false
    }
}

function Send-TagAssignmentSummaryTeamsNotification {
    param(
        [Parameter(Mandatory)][System.Collections.IEnumerable]$Rows,
        [string]$ResourceGroup = "Unknown",
        [string]$HostPoolName = "Unknown",
        [string]$AutomationRegion = "Unknown",
        [Parameter(Mandatory)][bool]$Enabled,
        [AllowEmptyString()][string]$WebhookUrl
    )
    ...
    Write-Host "[TEAMS] [$timestampUtc] Starting consolidated Teams notification. RowCount='$($summaryRows.Count)'."
    ...
    if ([string]::IsNullOrWhiteSpace($WebhookUrl)) {
        Write-Host "[WARNING] [$timestampUtc] Consolidated Teams notification was not sent because TEAMS_WEBHOOK_URL is empty."
        Write-Warning "Teams notification was not sent because TEAMS_WEBHOOK_URL is not configured. Tag-assignment processing will continue."
        return $false
    }

    if ($summaryRows.Count -eq 0) {
        Write-Host "[TEAMS] [$timestampUtc] No processed rows are available for the consolidated notification."
        return $false
    }

    $succeededCount = @($summaryRows | Where-Object { $_.Status -ieq "Succeeded" }).Count
    $failedCount    = @($summaryRows | Where-Object { $_.Status -ieq "Failed" }).Count
    $skippedCount   = @($summaryRows | Where-Object { $_.Status -ieq "Skipped" }).Count

    $bodyElements = New-Object System.Collections.Generic.List[object]

    $bodyElements.Add(@{
        type   = "TextBlock"
        text   = "AVD VM tag and user assignment"
        weight = "Bolder"
        size   = "Medium"
        wrap   = $true
    })
    ...
    $bodyElements.Add(@{
        ...
        facts = @(
            ...
            @{ title = "Host pool";      value = $HostPoolName },
            @{ title = "Region";         value = $AutomationRegion }
        )
    })
    ...
    $tableRows.Add(@{
        type  = "TableRow"
        cells = @(
            @{ type = "TableCell"; items = @(@{ type = "TextBlock"; text = "VM";            weight = "Bolder"; wrap = $true }) },
            @{ type = "TableCell"; items = @(@{ type = "TextBlock"; text = "Assigned user"; weight = "Bolder"; wrap = $true }) },
            @{ type = "TableCell"; items = @(@{ type = "TextBlock"; text = "Cost center";   weight = "Bolder"; wrap = $true }) },
            @{ type = "TableCell"; items = @(@{ type = "TextBlock"; text = "Status";        weight = "Bolder"; wrap = $true }) }
        )
    })

    foreach ($item in $summaryRows) {
        $vmName = if ([string]::IsNullOrWhiteSpace([string]$item.VmName)) { "Unknown" } else { [string]$item.VmName }
        $assignedUser = if ([string]::IsNullOrWhiteSpace([string]$item.AssignedUser)) { "Not assigned" } else { [string]$item.AssignedUser }
        $costCenter = if ([string]::IsNullOrWhiteSpace([string]$item.CostCenter)) { "Not set" } else { [string]$item.CostCenter }
        $status = if ([string]::IsNullOrWhiteSpace([string]$item.Status)) { "Unknown" } else { [string]$item.Status }

        $statusColor = switch ($status.ToLowerInvariant()) {
            "succeeded" { "Good" }
            "failed"    { "Attention" }
            "skipped"   { "Warning" }
            default     { "Default" }
        ...
        $tableRows.Add(@{
            type  = "TableRow"
            cells = @(
                @{ type = "TableCell"; verticalContentAlignment = "Center"; items = @(@{ type = "TextBlock"; text = $vmName;       wrap = $true }) },
                @{ type = "TableCell"; verticalContentAlignment = "Center"; items = @(@{ type = "TextBlock"; text = $assignedUser; wrap = $true }) },
                @{ type = "TableCell"; verticalContentAlignment = "Center"; items = @(@{ type = "TextBlock"; text = $costCenter;  wrap = $true }) },
                @{ type = "TableCell"; verticalContentAlignment = "Center"; items = @(@{ type = "TextBlock"; text = $status; color = $statusColor; weight = "Bolder"; wrap = $true }) }
            )
        })
    }
    ...
    foreach ($item in $summaryRows) {
        ...
        if ($status -ieq "Failed") {
            $vmName = if ([string]::IsNullOrWhiteSpace([string]$item.VmName)) { "Unknown" } else { [string]$item.VmName }
            $requestedUser = if ([string]::IsNullOrWhiteSpace([string]$item.RequestedUser)) { "Not provided" } else { [string]$item.RequestedUser }
            $requestedCostCenter = if ([string]::IsNullOrWhiteSpace([string]$item.RequestedCostCenter)) { "Not provided" } else { [string]$item.RequestedCostCenter }
            $failureResult = if ([string]::IsNullOrWhiteSpace([string]$item.Result)) { "No failure details were returned." } else { [string]$item.Result }
            ...
            $bodyElements.Add(@{
                ...
                items   = @(
                    @{
                        type   = "TextBlock"
                        text   = "Failure details for $vmName"
                        weight = "Bolder"
                        color  = "Attention"
                        wrap   = $true
                    },
                    @{
                        type = "FactSet"
                        facts = @(
                            @{ title = "Requested user";        value = $requestedUser },
                            @{ title = "Requested cost center"; value = $requestedCostCenter },
                            @{ title = "Result";                value = $failureResult }
                        )
                    }
                )
            })
    ...
    $bodyElements.Add(@{
        ...
        spacing   = "Medium"
        facts     = @(
            @{ title = "Processed"; value = [string]$summaryRows.Count },
            @{ title = "Succeeded"; value = [string]$succeededCount },
            @{ title = "Failed";    value = [string]$failedCount },
            @{ title = "Skipped";   value = [string]$skippedCount }
        )
    })
    ...
    try {
        Write-Host "[TEAMS] [$timestampUtc] Building compact consolidated Adaptive Card payload."
        $payloadJson = $payload | ConvertTo-Json -Depth 40 -Compress
        Write-Host "[TEAMS] [$timestampUtc] Sending one compact Teams notification for $($summaryRows.Count) VM(s)."
        ...
        Write-Host "[TEAMS] Compact consolidated Teams notification sent successfully for $($summaryRows.Count) VM(s)."
        return $true
    }
    catch {
        $notificationError = $_.Exception.Message
        Write-Host "[WARNING] Compact consolidated Teams notification could not be delivered. Error='$notificationError'."
        Write-Host "[WARNING] Tag and user assignment processing is not failed because Teams notification is best-effort only."
        Write-Warning "Teams notification could not be delivered. The Service Bus message and assignment results will not fail because of this notification error. Error='$notificationError'."
        return $false
    }
}
...
try {
    ...
    $automationRegion = Get-AutomationRegion
    $requestStatuses = Get-SettingList -Name "REQUEST_STATUSES" -DefaultValue "Submit,Submitted"
    $teamsNotificationEnabled = Get-BoolSetting -Name "TEAMS_NOTIFICATION_ENABLED" -DefaultValue $false
    $teamsWebhookUrl = Get-OptionalSetting -Name "TEAMS_WEBHOOK_URL"
    ...
    Write-Stage -Stage "CONFIG" -Message "AutomationRegion='$automationRegion'; InputCsvBlob='$inventoryBlob'; TeamsNotificationEnabled='$teamsNotificationEnabled'."
    if ($teamsNotificationEnabled -and [string]::IsNullOrWhiteSpace($teamsWebhookUrl)) {
        Write-Warning "TEAMS_NOTIFICATION_ENABLED is true but TEAMS_WEBHOOK_URL is missing. Assignment processing will continue and Teams notification will be skipped."
    }
    ...
    try {
        ...
        if ($requestRows.Count -eq 0) {
            ...
            $teamsSummaryRows = New-Object System.Collections.Generic.List[object]
            $summaryResourceGroups = New-Object System.Collections.Generic.List[string]
            $summaryHostPools = New-Object System.Collections.Generic.List[string]

            foreach ($row in $requestRows) {
                $vmName = Get-RowValue -Row $row -Column "VmName"
                $vmId = Get-RowValue -Row $row -Column "VmResourceId"
                $hostPoolName = Get-RowValue -Row $row -Column "HostPoolName"
                $resourceGroupName = Get-RowValue -Row $row -Column "ResourceGroup"
                $requestedUserForNotification = Get-RowValue -Row $row -Column "NewAssignedUser"
                $requestedCostCenterForNotification = Get-RowValue -Row $row -Column "new_az_res_costcenter"
                ...
                try {
                    ...
                    Assert-VmInAutomationRegion -Vm $vm -AutomationRegion $automationRegion
                    Write-Stage -Stage "REGION" -Message "VM '$vmName' passed regional validation for '$automationRegion'."

                    $requestedUser = (Get-RowValue -Row $row -Column "NewAssignedUser").Trim()
                    $requestedCostCenter = (Get-RowValue -Row $row -Column "new_az_res_costcenter").Trim()

                    Write-Host "[REQUEST] NewAssignedUser='$requestedUser'"
                    Write-Host "[REQUEST] new_az_res_costcenter='$requestedCostCenter'"
                    $actions = New-Object System.Collections.Generic.List[string]
                    $azureChangesApplied = $false

                    if (-not [string]::IsNullOrWhiteSpace($requestedCostCenter)) {
                        $currentCostCenter = Get-RowValue -Row $row -Column "az_res_costcenter"
                        ...
                        if ($currentCostCenter -eq $targetCostCenterForCompare -and $allTagTargetsAlreadyMatch) {
                            ...
                            Set-RowValue -Row $row -Column "az_res_costcenter" -Value $targetCostCenterForCompare
                    ...
                    if ([string]::IsNullOrWhiteSpace($requestedCostCenter) -and [string]::IsNullOrWhiteSpace($requestedUser)) {
                        Set-RequestResult -Row $row -Status "Skipped" -Message "No request values provided. Enter NewAssignedUser and/or new_az_res_costcenter, then set RequestStatus=Submit."
                    ...
                    $finalStatus = Get-RowValue -Row $row -Column "RequestStatus"
                    $finalMessage = Get-RowValue -Row $row -Column "RequestMessage"
                    $finalAssignedUser = Get-RowValue -Row $row -Column "AssignedUser"
                    $finalCostCenter = Get-RowValue -Row $row -Column "az_res_costcenter"

                    Write-Host "[TEAMS] Adding VM '$vmName' to the consolidated notification. Status='$finalStatus'; AssignedUser='$finalAssignedUser'; CostCenter='$finalCostCenter'."
                    $teamsSummaryRows.Add([pscustomobject]@{
                        VmName = $vmName
                        AssignedUser = $finalAssignedUser
                        CostCenter = $finalCostCenter
                        RequestedUser = $requestedUserForNotification
                        RequestedCostCenter = $requestedCostCenterForNotification
                        Status = $finalStatus
                        Result = $finalMessage
                    })
                    if (-not [string]::IsNullOrWhiteSpace($resourceGroupName)) { $summaryResourceGroups.Add($resourceGroupName) }
                    if (-not [string]::IsNullOrWhiteSpace($hostPoolName)) { $summaryHostPools.Add($hostPoolName) }
                    ...
                    Set-RowValue -Row $row -Column "new_az_res_costcenter" -Value ""
                    ...
                    $failedAssignedUser = Get-RowValue -Row $row -Column "AssignedUser"
                    $failedCostCenter = Get-RowValue -Row $row -Column "az_res_costcenter"
                    Write-Host "[TEAMS] Adding failed VM '$vmName' to the consolidated notification."
                    $teamsSummaryRows.Add([pscustomobject]@{
                        VmName = $vmName
                        AssignedUser = $failedAssignedUser
                        CostCenter = $failedCostCenter
                        RequestedUser = $requestedUserForNotification
                        RequestedCostCenter = $requestedCostCenterForNotification
                        Status = "Failed"
                        Result = $message
            ...
            $distinctResourceGroups = @($summaryResourceGroups | Select-Object -Unique)
            $distinctHostPools = @($summaryHostPools | Select-Object -Unique)
            $summaryResourceGroup = if ($distinctResourceGroups.Count -eq 1) { $distinctResourceGroups[0] } elseif ($distinctResourceGroups.Count -gt 1) { "Multiple" } else { "Unknown" }
            $summaryHostPool = if ($distinctHostPools.Count -eq 1) { $distinctHostPools[0] } elseif ($distinctHostPools.Count -gt 1) { "Multiple" } else { "Unknown" }

            Write-Host "[TEAMS] Preparing one consolidated notification for $($teamsSummaryRows.Count) processed VM row(s)."
            Send-TagAssignmentSummaryTeamsNotification `
                -Rows $teamsSummaryRows `
                -ResourceGroup $summaryResourceGroup `
                -HostPoolName $summaryHostPool `
                -AutomationRegion $automationRegion `
                -Enabled $teamsNotificationEnabled `
                -WebhookUrl $teamsWebhookUrl | Out-Null
