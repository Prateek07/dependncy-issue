...
function Send-CleanupTeamsNotification {
    ...
    Write-Host "[TEAMS] [$timestampUtc] Starting Teams notification processing. VmName='$VmName'; Status='$normalizedStatus'."
...
function Process-DeletedVmEvent {
    ...
    if (-not [string]::IsNullOrWhiteSpace($eventResourceLocation)) {
        ...
        if (-not $eventResourceLocation.Equals(
            ...
            $issueMessage = "Skipped VM deletion because event location '$eventResourceLocation' does not match Function App region '$script:AutomationRegion'."
            Write-LogWarning "$issueMessage VmName='$($vmInfo.VmName)', ResourceGroup='$($vmInfo.ResourceGroup)'."
        ...
        if (-not $resourceGroupMatchesRegion) {
            $issueMessage = "Skipped VM deletion because resource group '$($vmInfo.ResourceGroup)' does not match Function App region '$script:AutomationRegion'."
